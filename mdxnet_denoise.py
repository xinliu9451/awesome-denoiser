...
dict_args = {'model_path': 'example_data/UVR-MDX-NET-Inst_HQ_1.onnx', 'denoise': True, 'margin': 44100, 'chunks': 15, 'n_fft': 6144, 'dim_t': 8, 'dim_f': 3072}
predictor = Predictor(args=dict_args)

def mdxnet_denoise(audio_input1, audio_input2, dB, min_silence_duration):
    start = time.time()
    denoised1, no_vocals1, sr1 = predictor.predict(audio_input1)
    denoised1 = denoised1.mean(axis=1)
    if audio_input2 is not None:
        denoised2, no_vocals2, sr2 = predictor.predict(audio_input2)
        denoised2 = denoised2.mean(axis=1)
